...
def hand(arr)
	...
	arr.each do |x|
		temp = x.split("")
		if cards[@values[temp[0]]] == nil
			cards[@values[temp[0]]] = 1
		else
			cards[@values[temp[0]]] += 1
		end
		if suits[temp[1]] == nil
	...
	if len == 5
		...
		cards.each do |x, y|
			if counter == 5
				...
				if check == true
					temp.clear
					temp << @strengths["SFlush"]
					cards.each do |a, b|
						temp << a
					end
				else
					temp << @strengths["Straight"]
					check = true
					cards.each do |a, b|
						temp << a
					end
			...
			if cards[counter] == nil
				break
			elsif x + 1 == cards[counter][0]
				counter += 1
			else
				break
			end
	...
	if check == false
		...
		when 4
			temp << @strengths["One"] #The number forming the pair goes in correctly, the other come out in the format 2,1,3, unless the pair is higher than the next highest
			arr = []
			cards.each do |a, b|
				if b == 2
					temp << a
...
0.upto(p1.length-1) do |x|
	h1 = hand(p1[x])
	h2 = hand(p2[x])
	0.upto(h1.length) do |i|
		if h1[i] == nil
			others += 1
			break
		elsif h1[i] > h2[i]
